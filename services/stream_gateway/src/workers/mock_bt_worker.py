"""Mock BT worker for local stream gateway.
...
class MockBtWorker:
    """Simulates a BT sequential-download job timeline."""
    ...
    def create_job(
        ...
        *,
        ...
        base_job = {
            ...
            'status': 'queued',
            'progressPercent': 0,
            'message': 'Queued for metadata fetch.',
    ...
    def retry_job(self, job_id: str) -> dict[str, Any] | None:
        with self._lock:
            ...
            if attempt >= max_retries:
                base['canRetry'] = False
                base['message'] = 'Retry limit reached for bt worker.'
    ...
    def cancel_job(self, job_id: str) -> dict[str, Any] | None:
        with self._lock:
            ...
            base['message'] = 'BT job canceled by request.'
            base['canRetry'] = False
            base['updatedAt'] = self._to_iso(time.time())
            wrapper['base'] = base
            wrapper['terminal'] = True
    ...
    def _evaluate_job_locked(self, wrapper: dict[str, Any]) -> dict[str, Any]:
        ...
        if elapsed < 1.0:
            ...
            status = 'downloading'
            progress = 58
            message = 'Downloading sequential pieces.'
        elif elapsed < 6.5:
            status = 'buffering'
            progress = 82
            message = 'Building initial playback buffer.'
        else:
            status = 'completed'
            progress = 100
            message = 'BT worker completed initial buffer.'

        failure_mode = self._normalize_failure_mode(str(wrapper.get('failureMode', 'none')))
        should_fail = False
        if status in {'downloading', 'buffering'}:
        ...
        if should_fail:
            status = 'failed'
            progress = 46
            message = 'BT worker simulated failure during piece fetch.'
        ...
        if status == 'completed':
            base['outputRef'] = f"bt://{base.get('jobId', 'unknown')}/buffer"
            wrapper['terminal'] = True
        elif status == 'failed':
            base['errorCode'] = 'BT_SIMULATED_FAILURE'
            wrapper['terminal'] = True
